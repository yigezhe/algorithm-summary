...
struct node {
    int val;//value
    struct node *lch,*rch;//left child,right child
};
...
struct node *remove_it(struct node *p,int x) {
	//空的不用删除！
    if(p==NULL) return NULL;
    //从左支去删除！
    else if(x<p->val) p->lch=remove_it(p->lch,x);
    //从右支去删除！
    else if(x>p->val) p->rch=remove_it(p->rch,x);
    //下面的情况是已经找到了要删除的元素！
    else if(p->lch==NULL) {
	    //要删除的元素没有左支！
        struct node *q=p->rch;//保存整个右支！
        //delete p;//为什么要删除p？
	free(p);
	p=NULL;
        return q;//返回已经保存的右支？
	    ...
	    //有左儿子，但左儿子没有右儿子
        struct node *q=p->lch;//保存左儿子，左儿子的左支不变，右支变为被删除元素的右支！
        q->rch=p->rch;
        //delete p;
	free(p);
	p=NULL;
